...
#+END_SRC
** TODO dup
=dup= is duplicates an item on the stack. If the
item is a patchwerk cable, a dup operation happens
on the buffer stack as well.
** TODO drop
=drop= drops an item on the stack. If the item is a
patchwerk cable, a drop operation happens on
the buffer stack as well.
** TODO swap
