...
#+END_SRC
** Loading Raw Table Information
Raw table data can be loaded from disk
using =sk_core_tabload=. This will read the file into
a newly allocated table that then gets pushed onto the
stack.
