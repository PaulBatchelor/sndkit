...
int gf_node_cables_alloc(gf_node*node,int ncables)
{
    int n;
    int rc;
    if(node->patch==NULL)return GF_NOT_OK;
...
int gf_node_set_block(gf_node*node,int id)
{
    ...
    rc= gf_cable_make_block(
        &node->cables[id],
        gf_patch_stack(node->patch),
        node->blksize);
...
{
    int i;
    if(!gf_cable_is_block(in)||!gf_cable_is_block(sum))return GF_NOT_OK;
...
{
    if(stack->pos==0)return GF_NOT_OK;
...
{
    gf_buffer*tmp;
    if(stack->pos<2)return GF_NOT_OK;
...
int gf_patch_new_node(gf_patch*patch,gf_node**node)
{
    ...
    if(patch==NULL)return GF_NULL_VALUE;
...
void gf_patch_holdbuf(gf_patch*patch,gf_cable*c)
{
    ...
    if(gf_cable_is_constant(c))return;
